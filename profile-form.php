...
<div class="login profile" id="theme-my-login<?php $template->the_instance(); ?>">
	<form id="your-profile" class="stand-alone-form" action="" method="post">
		...
		<div class="form-row">
			<label for="user_login"><?php _e( 'Username', 'theme-my-login' ); ?> <span class="tip" onmouseover="tooltip('<?php _e( 'Your username cannot be changed.', 'hhh-theme' ); ?>');" onmouseout="exit();">?</span></label>
		...
		?>
			...
			<div class="form-row">
				<p class="description"><?php _e( 'If you would like to change the password type a new one. Otherwise leave this blank.', 'hhh-theme' ); ?></p>
				<label for="pass1"><?php _e( 'New Password', 'theme-my-login' ); ?> <span class="tip" onmouseover="tooltip('<?php _e( 'Hint: The password should be at least seven characters long. To make it stronger, use upper and lower case letters, numbers and symbols.', 'hhh-theme' ); ?>');" onmouseout="exit();">?</span></label>
				<input type="password" name="pass1" id="pass1" size="16" value="" autocomplete="off" />
			</div><div class="form-row">
				<label for="pass1"><?php _e( 'Repeat Password', 'hhh-theme' ); ?></label>
		...
		<div class="form-row check-row column-row">
			...
			<label for="deleteme" class="warning"><span class="box"></span><?php _e( 'Delete my account permanently', 'hhh-theme' ); ?></label>
		</div>
		<div class="form-row">
			<input type="hidden" name="user_id" id="user_id" value="<?php echo esc_attr( $current_user->ID ); ?>" />
			<input type="submit" onclick="
				if( jQuery('#deleteme').is(':checked') ) {
					if( confirm('<?php _e( "Do you really want to delete your account? This will be permanent and cannot be undone!", 'hhh-theme' ); ?>') ) {
						return true;
					} else {
						return false;
					}
				} else if( jQuery('#birthday-year').val() <= <?php echo( (intval(date('Y')) - 100) ); ?> ) {
					if( confirm('<?php _e( 'Are you really a hundred years old???.', 'hhh-theme' ); ?>') ) {
						return true;
					} else {
						return false;
					}
				} else if( jQuery('#birthday-year').val() == 1970 && jQuery('#birthday-month').val() == 1 && jQuery('#birthday-day').val() == 1 ) {
					if( confirm('<?php _e( 'Have you really been born on January 1st, 1970?.', 'hhh-theme' ); ?>') ) {
						return true;
					} else {
						return false;
					}
